...
3. 编排器按 10 阶段顺序执行，每阶段产物即时落盘 `data/runs/{run_id}/`
...
- `frontend/` 独立 Vite 工程；`npm run build` 产物输出到 `backend/app/static/`，FastAPI 静态托管。
- 开发模式支持 Vite dev server 代理 `/api`。
...
| M3 | 编排器 + SSE + 10 阶段框架（先空实现） | UI 可见进度流转 |
