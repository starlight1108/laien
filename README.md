...
- 🧠 **内置多 LLM 提供商**（OpenAI / Anthropic / DeepSeek / Gemini / 本地 Ollama / 自定义），选择后填写 Key 即可使用；Key 仅存内存不落盘
...
- 🔒 **模型配置持久化**：提供商 / Base URL / 模型名 / API Key 自动保存到浏览器本地（localStorage），刷新不丢失；Key 仅存本机浏览器，不上传后端存储
...
**安全**：Key 经请求头发送，仅存于**本次运行的后端内存**；禁止写入 `.env`、日志、缓存、数据库。环境变量仅作无 UI 兜底。
